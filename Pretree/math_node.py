...
class MathNode:
    ...
    def __init__(self, left, right, token):
        ...
        :return: None
        """
        if not isinstance(left, (variable_node.VariableNode,math_node.MathNode,literal_node.LiteralNode)):
            raise syntax_error.SyntaxError('Illegal Epression.')

        if not isinstance(right,(variable_node.VariableNode,math_node.MathNode,literal_node.LiteralNode)):
            raise syntax_error.SyntaxError('Illegal Epression.')
